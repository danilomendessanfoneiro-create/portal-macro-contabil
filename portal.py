...
def carregar_dados(cnpj, comp):
    ...
    # 1️⃣ PRIMEIRO TENTA NOTAS DETALHADAS
    ...
    # 2️⃣ FALLBACK → TOTAIS FATURAMENTO
    ...
    if (vals["fat"] == 0 and vals["ent"] == 0) and not df_t.empty:
        ...
        if "situacao" in df_t.columns:
            df_t = df_t[df_t["situacao"].astype(str).str.strip() == "Emitido"]
        ...
        if "tipoMovimento" in df_t.columns:
            ...
            # caso a API venha bugada (tipoMovimento ausente)

            mask_saida = df_t.apply(lambda r: "Saída" in str(r.values) or "Prestado" in str(r.values), axis=1)

            mask_ent = df_t.apply(lambda r: "Entrada" in str(r.values) or "Tomado" in str(r.values), axis=1)

            vals["fat"] = df_t[mask_saida]["valorTotal"].sum()

            vals["ent"] = df_t[mask_ent]["valorTotal"].sum()
